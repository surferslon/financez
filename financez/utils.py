...
def make_account_tree(section=None):
    if section:
        accounts = Account.objects.filter(results=section).values('pk', 'parent_id', 'name', 'order', 'acc_type', 'results').order_by('order')
        acc_list = [acc for acc in accounts]
        return add_subaccounts(acc_list, filter(lambda x: x['parent_id'] is None, acc_list))
    else:
        accounts = Account.objects.all().values('pk', 'parent_id', 'name', 'order').order_by('order')
        acc_list = [acc for acc in accounts]
        return add_subaccounts(acc_list, filter(lambda x: x['parent_id'] is None, acc_list))
